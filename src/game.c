...
int		fin_de_jeu_graphique(principal *donnee)
{
	...
	MLV_draw_text_box((LARG_WIN + 200) / 2 - 150, (HAUT_WIN / 2) + 50, 50, 50,"Oui", 0, MLV_COLOR_BLACK, MLV_COLOR_GREEN, MLV_COLOR_WHITE, MLV_TEXT_CENTER, MLV_HORIZONTAL_CENTER, MLV_VERTICAL_CENTER);
	MLV_draw_text_box(((LARG_WIN + 200) / 2) + 100, (HAUT_WIN / 2) + 50, 50, 50, "Non", 0, MLV_COLOR_BLACK, MLV_COLOR_RED, MLV_COLOR_WHITE, MLV_TEXT_CENTER, MLV_HORIZONTAL_CENTER, MLV_VERTICAL_CENTER);
	MLV_actualise_window();
	do{
		MLV_wait_mouse(&(donnee->jeu.coordonnee1[0]), &(donnee->jeu.coordonnee1[1]));
		if (donnee->jeu.coordonnee1[0] >= ((LARG_WIN + 200) / 2) - 150 && donnee->jeu.coordonnee1[0] < ((LARG_WIN + 200) / 2) - 100 && donnee->jeu.coordonnee1[1] >= (HAUT_WIN / 2) + 50 && donnee->jeu.coordonnee1[1] < (HAUT_WIN / 2) + 100)
			break ;
		else if (donnee->jeu.coordonnee1[0] >= ((LARG_WIN + 200) / 2) + 100 && donnee->jeu.coordonnee1[0] < ((LARG_WIN + 200) / 2) + 150 && donnee->jeu.coordonnee1[1] >= (HAUT_WIN / 2) + 50 && donnee->jeu.coordonnee1[1] < (HAUT_WIN / 2) + 100)
			return (0);
	}while (1);
	y = HAUT_WIN / 2 + 50;
	x = LARG_WIN / 2 + 125;
...
int		sauvegarde_tab(principal *donnee)
{
	...
	{
		save_path = ft_strjoin("save/", donnee->entrer.fichier, 0);
